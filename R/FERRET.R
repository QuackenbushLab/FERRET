...
BuildComparisonObject <- function(sourceNetwork, ingroupToCompare, outgroupToCompare,
  ...
  comparisons <- NULL
  if(!methods::is(results, "FERRET_Results") || !methods::is(sourceNetwork, "character") || 
     !methods::is(ingroupToCompare, "character") || !methods::is(outgroupToCompare, "character")){
...
ComputeRobustnessAUC <- function(results, comparisons, metric = c("jaccard", "in-degree", "out-degree"), 
  ...
  # Check the inputs.
  if(!methods::is(results, "FERRET_Results") || !methods::is(comparisons, "FERRET_Comparisons")){
...
ScaleNetworksByPercentile <- function(networks, numberOfCutoffs){
  ...
  allScaledNetworks <- lapply(networks, function(network){
    ...
    networkScaled <- network
    if(length(unique(network[which(network[,3] > 0),3])) > 0){
      quantilesPos <- stats::quantile(network[which(network[,3] >= 0),3], percentiles)
...
#' This function generates separate bar plot of the performance for each cell
#' type across validation methods.
#' @param resultDf A data frame with the following format:
#' Method, CellType, Score, Statistic, Jaccard, Ingroup, Outgroup
#' @param metric Metric used for network similarity
#' @returns NULL
#' @export
MakePerformanceBarPlot <- function(resultDf, metric){
  
  # Subset to include only the metric.
  resultDfForMetric <- resultDf[,c("Method", "CellType", "Score", "Statistic", metric)]
  colnames(resultDfForMetric) <- c("Method", "CellType", "Score", "Statistic", "Average")
  
  # Make the mins and maxes a separate column.
  resultDfForMetricAvg <- resultDfForMetric[which(resultDfForMetric$Statistic == "mean"),]
  resultDfForMetricMin <- resultDfForMetric[which(resultDfForMetric$Statistic == "min"),]
  resultDfForMetricMax <- resultDfForMetric[which(resultDfForMetric$Statistic == "max"),]
  resultDfForMetricAvg$Min <- resultDfForMetricMin$Average
  resultDfForMetricAvg$Max <- resultDfForMetricMax$Average
  
  # Generate the plots for each cell type.
  cellTypes <- unique(resultDfForMetricAvg$CellType)
  plotsPerCellType <- lapply(cellTypes, function(type){
    
    # Subset.
    resultDfCellType <- resultDfForMetricAvg[which(resultDfForMetricAvg$CellType == type),]
    
    # Set the width.
    resultDfCellType$width <- 0.5
    resultDfCellType$width[which(resultDfCellType$Method == "Monotonicity")] <- 0.25
    
    # Generate plot.
    return(ggplot2::ggplot(resultDfCellType, ggplot2::aes(x = resultDfCellType$Method, 
                                                 y = resultDfCellType$Average, fill = resultDfCellType$Score)) +
             ggplot2::geom_bar(stat = "identity", position = "dodge") +
             ggplot2::geom_errorbar(ggplot2::aes(ymin = resultDfCellType$Min, ymax = resultDfCellType$Max), 
             ...
             ggplot2::theme(plot.margin = ggplot2::margin(0.01, 0.01, 0, 0.01, "cm"),
                            plot.title = ggplot2::element_text(size = 10, face = "bold")) +
             ggplot2::scale_fill_manual(values = c("Monotonicity" = "gold3", "RAUC" = "royalblue")))
  })

  # Make the plot.
  gridExtra::grid.arrange(grobs = plotsPerCellType, ncol = 1)
}

#' This function returns the range of AUC scores and Monotonicity scores.
#' It also saves to a file if specified.
#' @param resultList A list of FERRET_ROC_AUC objects over which to compute ranges.
#' @returns A data frame with the following format:
#' Score, Statistic, Jaccard, Ingroup, Outgroup
#' RAUC, min, , ,
#' RAUC, max, , ,
#' RAUC, average, , ,
#' Monotonicity, min, , ,
#' Monotonicity, max, , ,
#' Monotonicity, average, , ,
#' @export
GetResultRanges <- function(resultList){
  ...
  # Find the average RAUC.
  averageResultsRauc <- as.data.frame(lapply(names(resultList[[1]]@auc), function(metric){
    results <- unlist(lapply(resultList, function(result){
      return(result@auc[[metric]])
    }))
    return(mean(results[which(!is.na(results))]))
  }))
  names(averageResultsRauc) <- names(resultList[[1]]@auc)

  # Find the min RAUC.
  minResultsRauc <- as.data.frame(lapply(names(resultList[[1]]@auc), function(metric){
    results <- unlist(lapply(resultList, function(result){
      return(result@auc[[metric]])
    }))
    return(min(results[which(!is.na(results))]))
  }))
  names(minResultsRauc) <- names(resultList[[1]]@auc)

  # Find the max RAUC.
  maxResultsRauc <- as.data.frame(lapply(names(resultList[[1]]@auc), function(metric){
    results <- unlist(lapply(resultList, function(result){
      return(result@auc[[metric]])
    }))
    return(max(results[which(!is.na(results))]))
  }))
  names(maxResultsRauc) <- names(resultList[[1]]@auc)
  
  # Find the average Monotonicity.
  averageResultsMono <- as.data.frame(lapply(names(resultList[[1]]@monotonicity), function(metric){
    results <- unlist(lapply(resultList, function(result){
      return(result@monotonicity[[metric]])
    }))
    return(mean(results[which(!is.na(results))]))
  }))
  names(averageResultsMono) <- names(resultList[[1]]@monotonicity)
  
  # Find the min Monotonicity.
  minResultsMono <- as.data.frame(lapply(names(resultList[[1]]@monotonicity), function(metric){
    results <- unlist(lapply(resultList, function(result){
      return(result@monotonicity[[metric]])
    }))
    return(min(results[which(!is.na(results))]))
  }))
  names(minResultsMono) <- names(resultList[[1]]@monotonicity)
  
  # Find the max Monotonicity.
  maxResultsMono <- as.data.frame(lapply(names(resultList[[1]]@monotonicity), function(metric){
    results <- unlist(lapply(resultList, function(result){
      return(result@monotonicity[[metric]])
    }))
    return(max(results[which(!is.na(results))]))
  }))
  names(maxResultsMono) <- names(resultList[[1]]@monotonicity)
  
  # Build the data frame.
  consolidated <- do.call(rbind, list(minResultsRauc, averageResultsRauc, maxResultsRauc,
                                      minResultsMono, averageResultsMono, maxResultsMono))
  consolidated$Score <- c(rep("RAUC", 3), rep("Monotonicity", 3))
  consolidated$Statistic <- rep(c("min", "mean", "max"), 2)
  consolidated <- consolidated[,c("Score", "Statistic", names(resultList[[1]]@auc))]
  return(consolidated)
}
...
WriteRobustnessAUC <- function(results, fileName){
  ...
  if(!methods::is(results, "FERRET_ROC_AUC")){
...
#' @param network The input network, formatted as a matrix.
...
#' @param geneSymbols The full list of gene symbols, with possible duplicates.
#' @returns The pathway results formatted as a data frame.
#' @export
RunPathwayAnalysis <- function(network, pathways, geneSymbols){
  
  # Obtain the targeting score for each gene.
  differentialTargeting <- colSums(network)
  names(differentialTargeting) <- geneSymbols
  ...
  pathwayResultDf <- data.frame(pathway = c(), pval = c(), padj = c(), pvalErrorSD = c(),
                                enrichmentScore = c(), normalizedEnrichmentScore = c(),
                                remainingGeneCount = c(), leadingGenesDrivingEnrichment = c())
  if(length(pathwayResult$leadingEdge) > 0){
...
#' @returns A single network formatted as an adjacency matrix.
#' @export
GetFullNetworkFromPCANetwork <- function(pcNetwork, PC, zeroCutoff = 0.001){
  ...
  pcAdjacency <- as.matrix(igraph::as_adjacency_matrix(graph = igraph::graph_from_data_frame(d = pcNetworkRenamed), attr = "Weight"))
...
PlotROC <- function(averageSims, lowestSims = NULL, highestSims = NULL, auc, monotonicity,
  ...
  if(!methods::is(averageSims, "FERRET_Similarities")){
...
ObtainNetworkCutoffs <- function(results, numberOfCutoffs){
  ...
  if(!methods::is(results, "FERRET_Results")){
...
JaccardSim <- function(network1, network2){
  ...
  netShared <- data.frame(net1 = network1[sharedEdges,3], net2 = network2[sharedEdges,3])
  mins <- apply(netShared, 1, FUN = min)
  intersectionSum <- sum(mins)
  ...
  unionNet <- data.frame(net1 = rep(0, length(allEdges)),
                         net2 = rep(0, length(allEdges)))
  rownames(unionNet) <- allEdges
  unionNet[rownames(network1), "net1"] <- network1[,3]
  unionNet[rownames(network2), "net2"] <- network2[,3]
  maxes <- apply(unionNet, 1, FUN = max)
  unionSum <- sum(maxes)
  jaccard <- intersectionSum / unionSum

  # If one network is empty, the overlap is 0.
  if(length(sharedEdges) == 0){
...
InDegreeSim <- function(network1, network2){
  ...
  if(nrow(network1) == 0 && nrow(network2) == 0){
    ...
    net1Order <- as.matrix(net1Adj[nodeOrder, nodeOrder])
    net2Order <- as.matrix(net2Adj[nodeOrder, nodeOrder])
