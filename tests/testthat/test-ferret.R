...
test_that("[FERRET] PlotROC() function handles errors appropriately",{
  ...
  expect_error(PlotROC(averageSims = "sim", auc = 0.5, monotonicity = 0.5,
...
test_that("[FERRET] GetFullNetworkFromPCANetwork() function yields expected results",{
  ...
  expect_equal(fullNet["gene1","gene2"], from1to2)
  expect_equal(fullNet["gene4","gene3"], from4to3)
  expect_equal(max(fullNet[,3]), fullNet["gene2","gene3"])
})